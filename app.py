...
from engine.getMood import next_mood_batch, first_mood_batch
from engine.bookRetrieval import *
from engine.autoCorrect import * 
from engine.tfidfSearchEngine import vectorize_data as tfidf_vectorize, search_query as tfidf_search
...
def check_data():
    ...
    if starting_up:
        starting_up = False
        # Start the data loading process in a background thread
        thread = threading.Thread(target=load_app_data)
        thread.daemon = True
        thread.start()
...
def home():
    if book_status < 150 or mood_status < 150:
